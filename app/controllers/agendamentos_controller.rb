class AgendamentosController < ApplicationController
  ...
  def index
    @agendamentos = Agendamento.where(["nome LIKE ?", "%#{params[:search]}%"]).order("hora ASC").paginate(page: params[:page], per_page: 8)
  end
  ...
  private
    ...
    def agendamento_params
      params.require(:agendamento).permit(:nome, :atendente, :preco, :hora, :anotacao)
    end
end
