class PacientesController < ApplicationController
  ...
  # GET /pacientes.json
  def index    
    @pacientes = Paciente.where(["nome LIKE ?", "%#{params[:search]}%"]).order("nome ASC").paginate(page: params[:page], per_page: 8)
  end
  ...
  private
    ...
    def paciente_params
      params.require(:paciente).permit(:nome, :prontuario, :identidade, :cpf, :nascimento, :idade, :sexo, :email, :telefone, :ans, :operadora, :plano, :cartao, :usuario, :titular, :observacao)
    end
end
